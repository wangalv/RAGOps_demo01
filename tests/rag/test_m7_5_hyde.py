"""
M7.5 — HyDE (Hypothetical Document Embeddings) Test

Run from LangGraphLearning/:
    .venv/bin/python tests/rag/test_m7_5_hyde.py

Comparison per query:
  [Dense@3 baseline]        direct query embedding, top-3
  [Dense@20 → Reranked]     M7.3: section-level + reranking
  [HyDE@20 → Reranked]      M7.5: hypothetical doc embedding + reranking

The hypothetical document generated by GLM is printed under each query
so you can inspect whether the LLM used the right legal vocabulary.
"""
...
def main():
    ...
    print("\n" + "=" * 75)
    print("Comparison: Dense@3  vs  Dense@20→Reranked  vs  HyDE@20→Reranked")
    print("=" * 75)
    ...
    for i, query in enumerate(TEST_QUERIES, 1):
        ...
        # HyDE top-20 → rerank
        print(f"  [Calling GLM to generate hypothetical document...]")
        hyde_candidates, hypo_doc = hyde.search(query, top_k=CANDIDATE_N)
        print(f"  Hypothetical doc: \"{hypo_doc[:180]}{'...' if len(hypo_doc) > 180 else ''}\"")
        for rank, r in enumerate(hyde_candidates, 1):
            r["prev_rank"] = rank
        hyde_reranked = reranker.rerank(query, hyde_candidates, top_k=FINAL_K)
        print_table(f"HyDE@{CANDIDATE_N} → Reranked", hyde_reranked, "rerank_score")
